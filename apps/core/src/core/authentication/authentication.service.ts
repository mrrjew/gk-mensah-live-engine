...
export class AuthenticationService {
  ...
  async pingDatabase() {
    try {
      await this.drizzleService.onModuleInit();
  ...
  async createUser(payload: CreateUserDto) {
    try {
      ...
      if (payload.role == 'Admin') {
        payload.adminKey = await this.hashService.hashPassword(
          payload.adminKey,
        );
      }
      ...
      const userRole: UserRole = (
        validRoles.includes(payload.role as UserRole) ? payload.role : 'User'
      ) as UserRole;
  ...
  async validateUser(payload: any) {
    try {
      ...
      if (conditions.length === 0) {
        throw new BadRequestException(
          'You must provide an email, username, or phone number',
        );
      }
