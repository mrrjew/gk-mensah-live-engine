...
export class AdminGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        // Extract headers
        const adminKey = (req.headers['x-admin-key'] || req.headers['X-Admin-Key'])?.toString();
        const authHeader = (req.headers['authorization'] || req.headers['Authorization'])?.toString();
        const token = authHeader?.split(' ')[1];

        // Check for token
        if (!token) {
            throw new UnauthorizedException('Missing JWT token in Authorization header');
        }

        // Verify JWT and extract payload
        let payload: any;
        try {
            payload = jwt.verify(token, process.env.JWT_SECRET || 'default_secret_key') as any;
        } catch (err) {
        ...
        const hasValidAdminRole = payload.isAdmin === true || 
                                  (typeof role === 'string' && (role === 'Admin' || role === 'SuperAdmin'));

        if (!hasValidAdminRole) {
            throw new UnauthorizedException('JWT token missing admin role (requires Admin or SuperAdmin)');
        }

        // Check for admin key header
        if (!adminKey) {
            throw new UnauthorizedException('Missing admin key in x-admin-key header');
        }
        ...
        try {
            ...
            const isAdminKeyValid = await bcrypt.compare(adminKey, admin.adminKey || '');
