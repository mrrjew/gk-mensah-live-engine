...
export class SuperAdminGuard implements CanActivate {
  canActivate(context: ExecutionContext): boolean {
    ...
    const superAdminKey = (
      req.headers['x-super-admin-key'] || req.headers['X-Super-Admin-Key']
    )?.toString();
    const authHeader = (
    ...
    if (!superAdminKey) {
      throw new UnauthorizedException(
        'Missing Super Admin API key (x-super-admin-key header required)',
      );
    }
    ...
    if (!token) {
      throw new UnauthorizedException(
        'Missing JWT token in Authorization header (Bearer token required)',
      );
    }
    ...
    try {
      ...
      if (payload) {
        ...
        if (
          ...
          throw new UnauthorizedException(
            'JWT token must have SuperAdmin or Admin role',
          );
        }
      }
